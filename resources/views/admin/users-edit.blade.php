<x-admin-layout activePage="users">
    ...
    <div class="max-w-2xl">
        <form action="{{ route('admin.users.update', $user) }}" method="POST" class="bg-white rounded-2xl shadow-sm p-6 space-y-6" data-aos="fade-up" onsubmit="confirmSubmit(event, 'Simpan perubahan data user ini?')">
            @csrf @method('PUT')
            <div>
            ...
            <div x-data="{ showPassword: false }">
                ...
                <div class="relative">
                    <input :type="showPassword ? 'text' : 'password'" name="password" class="w-full rounded-lg border-slate-300 focus:border-emerald-500 focus:ring-emerald-500 pr-12" minlength="6">
                    <button type="button"
