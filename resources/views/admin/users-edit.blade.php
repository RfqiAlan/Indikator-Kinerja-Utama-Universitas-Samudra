<x-admin-layout activePage="users">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 lg:p-8 w-full relative overflow-hidden mb-6 lg:mb-8" data-aos="fade-up">
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-5">
            <div class="flex items-center gap-4 mb-2 md:mb-0">
                ...
                <div>
                    <h1 class="text-2xl lg:text-3xl font-extrabold text-slate-800 tracking-tight">Perbarui User</h1>
                    <p class="text-slate-500 text-sm font-medium mt-1">Mengedit data akun pengguna {{ $user->name }}.</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-2xl">
        <form action="{{ route('admin.users.update', $user) }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 lg:p-8 space-y-6 relative overflow-hidden" data-aos="fade-up" onsubmit="confirmSubmit(event, 'Simpan perubahan data user ini?')">
            @csrf @method('PUT')
            ...
            <div class="space-y-5 relative z-10">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $user->name) }}" class="w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-blue-500 focus:ring-blue-500 transition-colors" required>
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email', $user->email) }}" class="w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-blue-500 focus:ring-blue-500 transition-colors" required>
                ...
                <div x-data="{ showPassword: false }">
                    <label class="block text-sm font-bold text-slate-700 mb-2">Password Baru <span class="text-slate-400 font-medium text-xs ml-1">(Kosongkan jika tidak diubah)</span></label>
                    <div class="relative">
                        <input :type="showPassword ? 'text' : 'password'" name="password" class="w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-blue-500 focus:ring-blue-500 transition-colors pr-12" minlength="6" placeholder="Biar kosong bila tetap sama">
                        <button type="button"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 pt-2" x-data="{ role: '{{ old('role', $user->role) }}' }">
                    <div>
                        ...
                        <select name="role" x-model="role" class="w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-blue-500 focus:ring-blue-500 transition-colors" required>
                            <option value="user" {{ old('role', $user->role) == 'user' ? 'selected' : '' }}>User (Operator Fakultas)</option>
                            <option value="admin" {{ old('role', $user->role) == 'admin' ? 'selected' : '' }}>Admin (Sistem Universal)</option>
                            <option value="TimKerjaSama" {{ old('role', $user->role) == 'TimKerjaSama' ? 'selected' : '' }}>Tim Kerja Sama (Khusus IKU 5)</option>
                            <option value="TimKeuangan" {{ old('role', $user->role) == 'TimKeuangan' ? 'selected' : '' }}>Tim Keuangan (Khusus IKU 9)</option>
                            <option value="TimPerencanaan" {{ old('role', $user->role) == 'TimPerencanaan' ? 'selected' : '' }}>Tim Perencanaan (Khusus IKU 11, 12, 13)</option>
                        </select>
                    </div>
