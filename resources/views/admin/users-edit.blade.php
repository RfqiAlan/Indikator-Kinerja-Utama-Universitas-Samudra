        ...
        <form action="{{ route('admin.users.update', $user) }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 lg:p-8 space-y-6 relative overflow-hidden" data-aos="fade-up" onsubmit="confirmSubmit(event, 'Simpan perubahan data user ini?')">
            ...
            <div class="space-y-5 relative z-10">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $user->name) }}" class="w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-emerald-500 focus:ring-emerald-500 transition-colors" required>
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email', $user->email) }}" class="w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-emerald-500 focus:ring-emerald-500 transition-colors" required>
                ...
                <div x-data="{ showPassword: false }">
                    ...
                    <div class="relative">
                        <input :type="showPassword ? 'text' : 'password'" name="password" class="w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-emerald-500 focus:ring-emerald-500 transition-colors pr-12" minlength="6" placeholder="Biar kosong bila tetap sama">
                        <button type="button"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 pt-2">
                    <div>
                        ...
                        <select name="role" class="w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-emerald-500 focus:ring-emerald-500 transition-colors" required>
                            ...
                            <option value="admin" {{ old('role', $user->role) == 'admin' ? 'selected' : '' }}>Admin (Sistem Universial)</option>
                        </select>
                    </div>
                    <div>
