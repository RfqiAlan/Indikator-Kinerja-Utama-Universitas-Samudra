            ...
            <form action="{{ route('user.iku9.update', $iku9->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm p-6 space-y-6" data-aos="fade-up" onsubmit="confirmSubmit(event, 'Apakah Anda yakin ingin menyimpan data ini?')">
                ...
                <div class="border-t pt-6"><h3 class="font-semibold text-slate-800 mb-4">Sumber Pendapatan Non-UKT</h3>
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                        <div class="bg-blue-50 p-3 rounded-lg"><label class="block text-sm font-medium text-blue-700 mb-1">Hibah Riset (Rp)</label><input type="number" name="hibah_riset" x-model.number="hibah" value="{{ old('hibah_riset', $iku9->hibah_riset) }}" class="w-full rounded-lg border-blue-200" min="0"></div>
                        ...
                        <div class="bg-indigo-50 p-3 rounded-lg"><label class="block text-sm font-medium text-indigo-700 mb-1">Unit Bisnis (Rp)</label><input type="number" name="unit_bisnis" x-model.number="bisnis" value="{{ old('unit_bisnis', $iku9->unit_bisnis) }}" class="w-full rounded-lg border-indigo-200" min="0"></div>
