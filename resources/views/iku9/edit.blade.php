...
<body class="font-sans antialiased bg-white text-slate-900">
    <x-user-layout activeIku="IKU 9">
        ...
        <div class="py-6 max-w-4xl mx-auto" x-data="formIku9()">
            ...
            @endif
            <form action="{{ route('user.iku9.update', $iku9->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm p-6 space-y-6" data-aos="fade-up" onsubmit="confirmSubmit(event, 'Apakah Anda yakin ingin menyimpan data ini?')">
                @csrf @method('PUT')
                    ...
                    <div><label class="block text-sm font-medium text-slate-700 mb-1">Tahun <span class="text-rose-500">*</span></label><x-tahun-akademik-select :selected="$iku9->tahun_akademik" /></div>
                        <div>
                            ...
                            <select name="triwulan" class="w-full rounded-lg border-slate-300 focus:ring-blue-500" required>
                                ...
                                <option value="1" {{ old('triwulan', $iku9->triwulan) == 1 ? 'selected' : '' }}>Triwulan 1</option>
                                <option value="2" {{ old('triwulan', $iku9->triwulan) == 2 ? 'selected' : '' }}>Triwulan 2</option>
                                <option value="3" {{ old('triwulan', $iku9->triwulan) == 3 ? 'selected' : '' }}>Triwulan 3</option>
                                <option value="4" {{ old('triwulan', $iku9->triwulan) == 4 ? 'selected' : '' }}>Triwulan 4</option>
                            </select>
                        </div>
                ...
                @include("partials.lampiran-upload", ["ikuNumber" => 9, "existingLinks" => $iku9->lampiran_link ?? []])
                <div class="flex justify-end gap-3"><a href="{{ route('user.iku9.index') }}" class="px-4 py-2 text-slate-600">Batal</a><button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-semibold">Update</button></div>
            </form>
        </div>
        <script>
            function formIku9() {
                return {
                    ...
                    totalAset: {{ old('total_aset', $iku9->total_aset ?? 0) }},
                    risetInovasi: {{ old('pendapatan_riset_inovasi', $iku9->pendapatan_riset_inovasi ?? 0) }},
                    kerjasamaLayanan: {{ old('pendapatan_kerjasama_layanan', $iku9->pendapatan_kerjasama_layanan ?? 0) }},
                    usahaBisnis: {{ old('pendapatan_usaha_bisnis', $iku9->pendapatan_usaha_bisnis ?? 0) }},
                    dipaApbn: {{ old('pendapatan_dipa_apbn', $iku9->pendapatan_dipa_apbn ?? 0) }},
                    industri: {{ old('pendapatan_industri', $iku9->pendapatan_industri ?? 0) }},
                    danaAbadi: {{ old('dana_abadi', $iku9->dana_abadi ?? 0) }},
                    danaMasyarakat: {{ old('dana_masyarakat', $iku9->dana_masyarakat ?? 0) }},
                    alokasiRiset: {{ old('alokasi_riset', $iku9->alokasi_riset ?? 0) }},
                    alokasiDosen: {{ old('alokasi_kompetensi_dosen', $iku9->alokasi_kompetensi_dosen ?? 0) }},
                    alokasiLab: {{ old('alokasi_laboratorium', $iku9->alokasi_laboratorium ?? 0) }},
