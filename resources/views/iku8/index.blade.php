        ...
        <div class="py-6 space-y-6">
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border p-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="text-center p-4 bg-emerald-50 dark:bg-emerald-900/30 rounded-xl"><p class="text-sm text-emerald-600">Persentase IKU 8</p><p class="text-3xl font-bold text-emerald-700">{{ number_format($overallPercentage, 2) }}%</p></div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border overflow-hidden">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        <thead class="bg-slate-50"><tr><th class="px-6 py-3 text-left text-xs font-semibold uppercase">Tahun</th><th class="px-6 py-3 text-center text-xs font-semibold uppercase">SDM</th><th class="px-6 py-3 text-center text-xs font-semibold uppercase">Tim</th><th class="px-6 py-3 text-center text-xs font-semibold uppercase">Narasumber</th><th class="px-6 py-3 text-center text-xs font-semibold uppercase">Ahli</th><th class="px-6 py-3 text-center text-xs font-semibold uppercase">%</th><th class="px-6 py-3 text-center text-xs font-semibold uppercase">Aksi</th></tr></thead>
                        <tbody class="divide-y">@foreach($data as $item)<tr class="hover:bg-slate-50"><td class="px-6 py-4 text-sm">{{ $item->tahun_akademik }}</td><td class="px-6 py-4 text-center">{{ $item->total_sdm }}</td><td class="px-6 py-4 text-center">{{ $item->tim_penyusun }}</td><td class="px-6 py-4 text-center">{{ $item->narasumber }}</td><td class="px-6 py-4 text-center">{{ $item->ahli_hukum }}</td><td class="px-6 py-4 text-center"><span class="px-2.5 py-0.5 rounded-full text-xs font-medium {{ $item->persentase_iku8 >= 5 ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800' }}">{{ number_format($item->persentase_iku8, 2) }}%</span></td><td class="px-6 py-4 text-center"><div class="flex justify-center gap-2"><a href="{{ route('user.iku8.edit', $item) }}" class="text-cyan-600"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></a><form action="{{ route('user.iku8.destroy', $item) }}" method="POST" onsubmit="return confirm('Hapus?')">@csrf @method('DELETE')<button class="text-rose-600"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button></form></div></td></tr>@endforeach</tbody>
                    </table>
                </div>
                @else<div class="px-6 py-12 text-center"><p class="text-slate-500">Belum ada data. <a href="{{ route('user.iku8.create') }}" class="text-emerald-600 hover:underline">Tambah sekarang</a></p></div>@endif
            </div>
        </div>
    </x-user-layout>
</body>
