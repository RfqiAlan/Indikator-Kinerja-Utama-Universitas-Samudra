...
<body class="font-sans antialiased bg-white text-slate-900">
    <x-user-layout activeIku="IKU 11">
        <x-slot name="header">
            <div><h2 class="text-2xl font-bold text-slate-800">Tambah Data IKU 11</h2><p class="text-sm text-slate-500 mt-1">{{ auth()->user()->fakultas_nama ?? 'Fakultas' }} - Tata Kelola Perguruan Tinggi</p></div>
        </x-slot>
        <div class="py-6 max-w-4xl mx-auto" x-data="formIku11()">
            ...
            @endif
            <form action="{{ route('user.iku11.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm p-6 space-y-6" data-aos="fade-up" onsubmit="confirmSubmit(event, 'Apakah Anda yakin ingin menyimpan data ini?')">
                @csrf
                ...
                <div class="border-t pt-6">
                    ...
                    <select name="opini_audit" class="w-full md:w-1/2 rounded-lg border-slate-300">
                        ...
                        <option value="{{ $kode }}" {{ old('opini_audit') === $kode ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="border-t pt-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div class="bg-rose-50 p-3 rounded-lg">
                            ...
                            <input type="number" name="pelanggaran_plagiarisme" x-model.number="plagiarisme" value="{{ old('pelanggaran_plagiarisme', 0) }}" class="w-full rounded-lg border-rose-200" min="0" required>
                        </div>
                        <div class="bg-amber-50 p-3 rounded-lg">
                            <label class="block text-sm font-medium text-amber-700 mb-1">Fabrikasi</label>
                            <input type="number" name="pelanggaran_fabrikasi" x-model.number="fabrikasi" value="{{ old('pelanggaran_fabrikasi', 0) }}" class="w-full rounded-lg border-amber-200" min="0" required>
                        </div>
                        <div class="bg-orange-50 p-3 rounded-lg">
                            <label class="block text-sm font-medium text-orange-700 mb-1">Falsifikasi Data</label>
                            <input type="number" name="pelanggaran_falsifikasi" x-model.number="falsifikasi" value="{{ old('pelanggaran_falsifikasi', 0) }}" class="w-full rounded-lg border-orange-200" min="0" required>
                        </div>
                        <div class="bg-red-50 p-3 rounded-lg">
                            <label class="block text-sm font-medium text-red-700 mb-1">Penyalahgunaan Karya Ilmiah</label>
                            <input type="number" name="pelanggaran_penyalahgunaan" x-model.number="penyalahgunaan" value="{{ old('pelanggaran_penyalahgunaan', 0) }}" class="w-full rounded-lg border-red-200" min="0" required>
                        </div>
                        <div class="bg-pink-50 p-3 rounded-lg">
                            <label class="block text-sm font-medium text-pink-700 mb-1">Pelanggaran Etika Publikasi</label>
                            <input type="number" name="pelanggaran_etika_publikasi" x-model.number="etikaPublikasi" value="{{ old('pelanggaran_etika_publikasi', 0) }}" class="w-full rounded-lg border-pink-200" min="0" required>
                        </div>
                ...
                <div class="border-t pt-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="bg-blue-50 p-3 rounded-lg">
                            ...
                            <input type="number" name="kegiatan_direncanakan" x-model.number="direncanakan" value="{{ old('kegiatan_direncanakan', 0) }}" class="w-full rounded-lg border-blue-200" min="0" required>
                        </div>
                        <div class="bg-cyan-50 p-3 rounded-lg">
                            ...
                            <input type="number" name="kegiatan_terlaksana" x-model.number="terlaksana" value="{{ old('kegiatan_terlaksana', 0) }}" class="w-full rounded-lg border-cyan-200" min="0" required>
                        </div>
                    </div>
                ...
                @include("partials.lampiran-upload", ["ikuNumber" => 11])
                <div class="flex justify-end gap-3"><a href="{{ route('user.iku11.index') }}" class="px-4 py-2 text-slate-600">Batal</a><button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-semibold">Simpan</button></div>
            </form>
        </div>
        <script>
            function formIku11() {
                return {
                    plagiarisme: {{ old('pelanggaran_plagiarisme', 0) }},
                    fabrikasi: {{ old('pelanggaran_fabrikasi', 0) }},
                    falsifikasi: {{ old('pelanggaran_falsifikasi', 0) }},
                    penyalahgunaan: {{ old('pelanggaran_penyalahgunaan', 0) }},
                    etikaPublikasi: {{ old('pelanggaran_etika_publikasi', 0) }},
                    direncanakan: {{ old('kegiatan_direncanakan', 0) }},
                    terlaksana: {{ old('kegiatan_terlaksana', 0) }},
