...
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Tambah IKU 11</title>@vite(['resources/css/app.css', 'resources/js/app.js'])</head>
<body class="font-sans antialiased bg-white text-slate-900">
    <x-user-layout activeIku="IKU 11">
        ...
        <div class="py-6 max-w-4xl mx-auto">
            <form action="{{ route('user.iku11.store') }}" method="POST" class="bg-white rounded-2xl shadow-sm p-6 space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div><label class="block text-sm font-medium text-slate-700 mb-1">Nilai SAKIP</label><input type="number" name="nilai_sakip" step="0.01" min="0" max="100" value="{{ old('nilai_sakip') }}" class="w-full rounded-lg border-slate-300" placeholder="0 - 100"></div>
                    <div><label class="block text-sm font-medium text-slate-700 mb-1">Jumlah Pelanggaran</label><input type="number" name="jumlah_pelanggaran" min="0" value="{{ old('jumlah_pelanggaran', 0) }}" class="w-full rounded-lg border-slate-300" required></div>
                </div>
