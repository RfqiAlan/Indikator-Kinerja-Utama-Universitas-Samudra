...
<head>
    ...
    <title>{{ config('app.name', 'IKU UNSAM') }} - IKU 7: Keterlibatan SDGs</title>
    ...
    <x-user-layout activeIku="IKU 7">
        <x-slot name="header">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 w-full">
                <div>
                    <h2 class="text-xl font-bold text-black tracking-tight">IKU 7: Keterlibatan SDGs</h2>
                    <p class="text-sm font-medium text-slate-600 dark:text-slate-400 mt-1">Program dan kegiatan universitas yang berkontribusi pada Tujuan Pembangunan Berkelanjutan (SDGs).</p>
                </div>
                <div class="flex items-center gap-3">
                    <form method="GET" action="{{ route('user.iku7.index') }}" class="flex items-center gap-2">
        ...
        <div class="py-6 space-y-6" data-aos="fade-up">
            @php $target = 40; $meetsTarget = $overallPercentage >= $target; @endphp
            ...
            <div class="relative overflow-hidden bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-6 sm:p-8">
                <div class="absolute top-0 right-0 -mr-16 -mt-16 w-64 h-64 rounded-full bg-teal-50 dark:bg-teal-900/20 blur-3xl opacity-60"></div>
                <div class="absolute bottom-0 left-0 -ml-16 -mb-16 w-64 h-64 rounded-full bg-emerald-50 dark:bg-emerald-900/20 blur-3xl opacity-60"></div>
                <div class="relative flex flex-col md:flex-row items-center justify-between gap-8">
                    <div class="flex-1 text-center md:text-left space-y-2 max-w-lg">
                        <div class="flex items-center justify-center md:justify-start gap-2 mb-2">
                            <span class="px-2.5 py-0.5 rounded-full bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400 text-xs font-bold uppercase tracking-wide">IKU 7 Performance</span>
                        </div>
                        ...
                        <p class="text-lg font-medium text-slate-600 dark:text-slate-300">Rasio Program Mendukung SDGs (Agregat)</p>
                        <p class="text-sm text-slate-500 dark:text-slate-400">
                            Proporsi program/kegiatan yang berkontribusi pada SDGs dibanding total program universitas.
                        </p>
                        <div class="flex flex-wrap justify-center md:justify-start gap-3 mt-4">
                            <span class="inline-flex flex-col items-start px-3 py-2 rounded-lg bg-slate-50 text-slate-800 dark:bg-slate-900/30 dark:text-slate-300 border border-slate-200 shadow-sm">
                                <span class="text-[10px] uppercase font-bold text-slate-500">Total Program</span>
                                <span class="text-lg font-semibold">{{ number_format($totalProgram) }}</span>
                            </span>
                            <span class="inline-flex flex-col items-start px-3 py-2 rounded-lg bg-teal-50 text-teal-800 dark:bg-teal-900/30 dark:text-teal-300 border border-teal-100 shadow-sm">
                                <span class="text-[10px] uppercase font-bold text-teal-600">Program SDGs</span>
                                <span class="text-lg font-semibold">{{ number_format($totalProgramSdgs) }}</span>
                            </span>
                        </div>
                    </div>
                    <div class="relative w-40 h-40 flex items-center justify-center">
                        <svg class="transform -rotate-90 w-full h-full" viewBox="0 0 36 36">
                            ...
                            @php
                            $strokeColor = $meetsTarget ? 'text-teal-500' : 'text-rose-500';
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700">
                    <h3 class="text-lg font-semibold text-slate-800 dark:text-white">Data Keterlibatan SDGs</h3>
                </div>
                @if($data->count() > 0)
                    ...
                    <table class="w-full text-sm text-left whitespace-nowrap md:whitespace-normal">
                        <thead class="text-xs text-slate-500 uppercase bg-slate-50/80 dark:bg-slate-700/50 dark:text-slate-400 border-b border-slate-100 dark:border-slate-700">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-4 font-medium text-center">Program SDGs</th>
                                <th scope="col" class="px-6 py-4 font-medium text-center">Penelitian SDGs</th>
                                <th scope="col" class="px-6 py-4 font-medium text-center">Pengabdian SDGs</th>
                                <th scope="col" class="px-6 py-4 font-medium text-center">Capaian IKU 7</th>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50">
                            ...
                            <tr class="group hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 text-center text-slate-600 dark:text-slate-300">{{ number_format($item->total_program) }}</td>
                                <td class="px-6 py-4 text-center font-semibold text-teal-600">{{ number_format($item->total_program_sdgs) }}</td>
                                <td class="px-6 py-4 text-center text-slate-600 dark:text-slate-300">{{ number_format($item->penelitian_sdgs ?? 0) }}</td>
                                <td class="px-6 py-4 text-center text-slate-600 dark:text-slate-300">{{ number_format($item->pengabdian_sdgs ?? 0) }}</td>
                                <td class="px-6 py-4 text-center">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $item->persentase_iku7 >= 40 ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/50 dark:text-emerald-300' : 'bg-rose-100 text-rose-800 dark:bg-rose-900/50 dark:text-rose-300' }}">
                ...
                <div class="px-6 py-12 text-center">
                    ...
                    <p class="text-slate-500 max-w-sm mx-auto mb-6">Mulai dengan menambahkan data program yang berkontribusi pada target SDGs.</p>
                    <a href="{{ route('user.iku7.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 shadow-sm transition-all">
            ...
            <div x-data="{ open: false }" class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm overflow-hidden" data-aos="fade-up" data-aos-delay="200">
                <button @click="open = !open" class="w-full px-6 py-4 flex items-center justify-between text-left hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="font-bold text-slate-900 dark:text-white">Panduan & Rumus Perhitungan IKU 7</span>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-2">
                        <div class="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-xl p-5 border border-slate-300 dark:border-slate-600 shadow-sm hover:-translate-y-1 transition-transform duration-300">
                            <div class="flex items-center gap-2 mb-3">
                                ...
                                <h4 class="font-bold text-slate-800">Rumus IKU 7</h4>
                            </div>
                            <div class="p-3 bg-white rounded-lg text-xs font-mono text-slate-900 border border-slate-300 mb-3 text-center shadow-inner">
                                Total Program SDGs / Total Program × 100%
                            </div>
                            <p class="text-xs text-slate-500 leading-relaxed">Proporsi kegiatan tridharma (penelitian, pengabdian, pendidikan) yang berkaitan dengan target SDGs.</p>
                        </div>
                        <div class="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-xl p-5 border border-slate-300 dark:border-slate-600 shadow-sm hover:-translate-y-1 transition-transform duration-300">
                            <div class="flex items-center gap-2 mb-3">
                                <span class="flex items-center justify-center w-6 h-6 rounded-full bg-slate-100 font-bold text-xs">2</span>
                                <h4 class="font-bold text-slate-800">Cakupan SDGs</h4>
                            </div>
                            <ul class="space-y-1.5 text-xs text-slate-600">
                                <li class="flex gap-2"><span class="text-teal-600">●</span> Penelitian bertema SDGs</li>
                                <li class="flex gap-2"><span class="text-teal-600">●</span> Pengabdian masyarakat SDGs</li>
                                <li class="flex gap-2"><span class="text-teal-600">●</span> Kurikulum berbasis SDGs</li>
                                <li class="flex gap-2"><span class="text-teal-600">●</span> Kemitraan untuk SDGs</li>
                            </ul>
                        </div>
                        <div class="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-xl p-5 border border-slate-300 dark:border-slate-600 shadow-sm hover:-translate-y-1 transition-transform duration-300">
                            <div class="flex items-center gap-2 mb-3">
                            ...
                            <p class="text-xs text-slate-500 leading-relaxed">Minimal 40% dari seluruh program tridharma harus berkontribusi pada pencapaian SDGs.</p>
                        </div>
                    </div>
                </div>
            </div>
