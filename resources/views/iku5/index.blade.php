    ...
    <x-user-layout activeIku="IKU 5">
        <x-slot name="header">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-slate-800 dark:text-slate-100 tracking-tight antialiased">IKU 5: Rasio Luaran Kerja Sama</h2>
                    <p class="text-sm font-medium text-slate-600 dark:text-slate-400 mt-1 antialiased">Artikel kolaborasi, produk terapan, studi kasus, TTG, karya seni.</p>
                </div>
        ...
        <div class="py-6 space-y-6">
            @php $target = 10; $meetsTarget = $overallPercentage >= $target; $bgColor = $meetsTarget ? 'bg-emerald-50 dark:bg-emerald-900/30' : 'bg-rose-50 dark:bg-rose-900/30'; $textColor = $meetsTarget ? 'text-emerald-600' : 'text-rose-600'; $valueColor = $meetsTarget ? 'text-emerald-700' : 'text-rose-700'; @endphp
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50">
                            ...
                            <tr class="group hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $item->persentase_iku5 >= 10 ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/50 dark:text-emerald-300' : 'bg-rose-100 text-rose-800 dark:bg-rose-900/50 dark:text-rose-300' }}">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                        <a href="{{ route('user.iku5.edit', $item) }}" class="p-2 text-cyan-600 hover:bg-cyan-50 rounded-lg dark:text-cyan-400 dark:hover:bg-cyan-900/50 transition-colors" title="Edit">
                                        ...
                                        <form id="delete-iku5-{{ $item->id }}" action="{{ route('user.iku5.destroy', $item) }}" method="POST" class="inline">
                                            @csrf @method('DELETE')
                                            <button type="button" onclick="confirmDelete('delete-iku5-{{ $item->id }}')" class="p-2 text-rose-600 hover:bg-rose-50 rounded-lg dark:text-rose-400 dark:hover:bg-rose-900/50 transition-colors" title="Hapus">
                ...
                <div class="px-6 py-12 text-center">
                    ...
                    <a href="{{ route('user.iku5.create') }}" class="inline-flex items-center px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 shadow-sm transition-all">
