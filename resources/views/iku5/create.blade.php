            ...
            <form action="{{ route('user.iku5.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm p-6 space-y-6" data-aos="fade-up" onsubmit="confirmSubmit(event, 'Apakah Anda yakin ingin menyimpan data ini?')">
                ...
                <div class="border-t pt-6"><h3 class="font-semibold text-slate-800 mb-4">Jenis Luaran Kerja Sama</h3>
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                        <div class="bg-blue-50 p-3 rounded-lg"><label class="block text-sm font-medium text-blue-700 mb-1">Artikel Kolaborasi</label><input type="number" name="artikel_kolaborasi" x-model.number="artikel" value="{{ old('artikel_kolaborasi', 0) }}" class="w-full rounded-lg border-blue-200" min="0"></div>
                        ...
                        <div class="bg-indigo-50 p-3 rounded-lg"><label class="block text-sm font-medium text-indigo-700 mb-1">Studi Kasus</label><input type="number" name="studi_kasus" x-model.number="studi" value="{{ old('studi_kasus', 0) }}" class="w-full rounded-lg border-indigo-200" min="0"></div>
