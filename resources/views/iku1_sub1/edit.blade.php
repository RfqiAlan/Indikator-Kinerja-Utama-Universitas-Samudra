...
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Edit Sub IKU 1.1</title>@vite(['resources/css/app.css', 'resources/js/app.js'])
    <link href="https://unpkg.com/aos@2.3.1/dist/aos.css" rel="stylesheet"></head>
<body class="font-sans antialiased bg-white text-slate-900">
    <x-user-layout activeIku="IKU 1.1">
        <x-slot name="header">
            <div>
                <h2 class="text-2xl font-bold text-slate-800">Edit Data Sub IKU 1.1</h2>
                <p class="text-sm text-slate-500 mt-1">{{ auth()->user()->fakultas_nama ?? 'Fakultas' }} - Mahasiswa S2/S3 & Asing</p>
            </div>
        </x-slot>
        <div class="py-6 max-w-4xl mx-auto" x-data="formIku1Sub1Edit()">
            <form action="{{ route('user.iku1_sub1.update', $iku1Sub1) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm p-6 space-y-6" data-aos="fade-up" onsubmit="confirmSubmit(event, 'Apakah Anda yakin ingin mengubah data ini?')">
                @csrf
                @method('PUT')
                ...
                <div class="border-b pb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="tahun_akademik" value="{{ old('tahun_akademik', $iku1Sub1->tahun_akademik) }}" class="w-full rounded-lg border-slate-300 focus:ring-blue-500 bg-slate-50" readonly>
                        </div>
                        <div>
                            ...
                            <select name="triwulan" class="w-full rounded-lg border-slate-300 focus:ring-blue-500" required>
                                ...
                                <option value="1" {{ old('triwulan', $iku1Sub1->triwulan) == 1 ? 'selected' : '' }}>Triwulan 1</option>
                                <option value="2" {{ old('triwulan', $iku1Sub1->triwulan) == 2 ? 'selected' : '' }}>Triwulan 2</option>
                                <option value="3" {{ old('triwulan', $iku1Sub1->triwulan) == 3 ? 'selected' : '' }}>Triwulan 3</option>
                                <option value="4" {{ old('triwulan', $iku1Sub1->triwulan) == 4 ? 'selected' : '' }}>Triwulan 4</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="keterangan" rows="2" class="w-full rounded-lg border-slate-300">{{ old('keterangan', $iku1Sub1->keterangan) }}</textarea>
                </div>

                @include("partials.lampiran-upload", ["ikuNumber" => '1.1', "existingLinks" => $iku1Sub1->lampiran_link])
        ...
        <script>
            function formIku1Sub1Edit() {
                return {
                    totalMahasiswa: {{ old('total_mahasiswa_aktif', $iku1Sub1->total_mahasiswa_aktif) }},
                    mhsS2: {{ old('mahasiswa_aktif_s2', $iku1Sub1->mahasiswa_aktif_s2) }},
                    mhsS3: {{ old('mahasiswa_aktif_s3', $iku1Sub1->mahasiswa_aktif_s3) }},
                    mhsInternasional: {{ old('mahasiswa_internasional', $iku1Sub1->mahasiswa_internasional) }},
