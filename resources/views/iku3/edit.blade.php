...
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Edit IKU 3</title>@vite(['resources/css/app.css', 'resources/js/app.js'])</head>
<body class="font-sans antialiased bg-white text-slate-900">
    <x-user-layout activeIku="IKU 3">
        ...
        <div class="py-6 max-w-4xl mx-auto" x-data="formIku3()">
            ...
            <form action="{{ route('user.iku3.update', $iku3->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="border-b pb-6">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                        ...
                        <div class="bg-blue-50 p-3 rounded-lg">
                            ...
                            <input type="number" name="kkn_tematik" x-model.number="kkn" value="{{ old('kkn_tematik', $iku3->kkn_tematik) }}" class="w-full rounded-lg border-blue-200" min="0">
                        </div>
                        <div class="bg-indigo-50 p-3 rounded-lg">
                            <label class="block text-sm font-medium text-indigo-700 mb-1">Lomba/Kompetisi</label>
                            <input type="number" name="lomba" x-model.number="lomba" value="{{ old('lomba', $iku3->lomba) }}" class="w-full rounded-lg border-indigo-200" min="0">
                        </div>
                        <div class="bg-amber-50 p-3 rounded-lg">
                            <label class="block text-sm font-medium text-amber-700 mb-1">Wirausaha</label>
                            <input type="number" name="wirausaha" x-model.number="wirausaha" value="{{ old('wirausaha', $iku3->wirausaha) }}" class="w-full rounded-lg border-amber-200" min="0">
                        </div>
                    </div>
                </div>
                ...
                @include("components.lampiran-upload", ["existingLink" => $iku3->lampiran_link ?? null])
        ...
        <script>
            function formIku3() {
                return {
                    ...
                    kkn: {{ old('kkn_tematik', $iku3->kkn_tematik ?? 0) }}, 
