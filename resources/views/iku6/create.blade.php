...
<body class="font-sans antialiased bg-white text-slate-900">
    <x-user-layout activeIku="IKU 6">
        ...
        <div class="py-6 max-w-4xl mx-auto" x-data="formIku6()">
            <form action="{{ route('user.iku6.store') }}" method="POST" class="bg-white rounded-2xl shadow-sm p-6 space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div><label class="block text-sm font-medium text-slate-700 mb-1">Total Publikasi <span class="text-rose-500">*</span></label><input type="number" name="total_publikasi" x-model.number="totalPublikasi" class="w-full rounded-lg border-slate-300" required min="1"></div>
                </div>
                <div class="border-t pt-6"><h3 class="font-semibold text-slate-800 mb-4">Publikasi per Quartile</h3>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div class="bg-emerald-50 p-3 rounded-lg"><label class="block text-sm font-medium text-emerald-700 mb-1">Q1 (Bobot 1.0)</label><input type="number" name="publikasi_q1" x-model.number="q1" class="w-full rounded-lg border-emerald-200" min="0" value="0"></div>
                        <div class="bg-cyan-50 p-3 rounded-lg"><label class="block text-sm font-medium text-cyan-700 mb-1">Q2 (Bobot 0.75)</label><input type="number" name="publikasi_q2" x-model.number="q2" class="w-full rounded-lg border-cyan-200" min="0" value="0"></div>
                        <div class="bg-teal-50 p-3 rounded-lg"><label class="block text-sm font-medium text-teal-700 mb-1">Q3 (Bobot 0.50)</label><input type="number" name="publikasi_q3" x-model.number="q3" class="w-full rounded-lg border-teal-200" min="0" value="0"></div>
                        <div class="bg-blue-50 p-3 rounded-lg"><label class="block text-sm font-medium text-blue-700 mb-1">Q4 (Bobot 0.25)</label><input type="number" name="publikasi_q4" x-model.number="q4" class="w-full rounded-lg border-blue-200" min="0" value="0"></div>
                    </div>
                    <div class="mt-4"><label class="flex items-center"><input type="checkbox" name="kolaborasi_internasional" x-model="kolaborasi" class="rounded border-slate-300 text-emerald-600 focus:ring-emerald-500"><span class="ml-2 text-sm text-slate-700">Kolaborasi Internasional (+0.25 bonus)</span></label></div>
                </div>
        ...
        <script>
            function formIku6() {
                return { totalPublikasi: 0, q1: 0, q2: 0, q3: 0, q4: 0, kolaborasi: false,
                    get skorPublikasi() { let skor = (this.q1 * 1) + (this.q2 * 0.75) + (this.q3 * 0.5) + (this.q4 * 0.25); return this.kolaborasi ? skor + 0.25 : skor; },
