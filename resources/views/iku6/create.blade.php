...
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Tambah IKU 6</title>@vite(['resources/css/app.css', 'resources/js/app.js'])
...
<body class="font-sans antialiased bg-white text-slate-900">
    <x-user-layout activeIku="IKU 6">
        ...
        <div class="py-6 max-w-4xl mx-auto" x-data="formIku6()">
            ...
            @endif
            <form action="{{ route('user.iku6.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm p-6 space-y-6" data-aos="fade-up" onsubmit="confirmSubmit(event, 'Apakah Anda yakin ingin menyimpan data ini?')">
                @csrf
                ...
                <div class="border-t pt-6"><h3 class="font-semibold text-slate-800 mb-4">Publikasi per Quartile</h3>
                    <div class="grid grid-cols-2 md:grid-cols-6 gap-4">
                        ...
                        <div class="bg-violet-50 p-3 rounded-lg"><label class="block text-sm font-medium text-violet-700 mb-1">Prosiding (Bobot 0.25)</label><input type="number" name="prosiding_internasional" x-model.number="prosiding" value="{{ old('prosiding_internasional', 0) }}" class="w-full rounded-lg border-violet-200" min="0"></div>
                    </div>
                    <div class="mt-4">
